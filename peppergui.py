...
class PepperControllerApp:
    ...
    def on_connect_clicked(self):
        ...
        if self.robot == None:
            ...
            self.output_text("[INFO]: Robot is initialized at " + self.ip_address + ":" + str(port))
    ...
    def on_app_clicked(self, widget_id):
        self.output_text("[INFO]: Running app: " + self.configuration.conf[widget_id]["name"] + ".")
        self.robot.start_behavior(self.configuration.conf[widget_id]["package"])
        
        
    def on_gesture_clicked(self, widget_id):
        self.output_text("[INFO]: Doing gesture: " + self.configuration.conf[widget_id]["name"] + ".")
        path = np.random.choice(self.configuration.conf[widget_id]["path_list"])
    ...
    def animation_from_path(self, path):
        try:
            ...
            animation_finished = self.robot.animation_service.run("animations/[posture]/" + path, _async=True)
