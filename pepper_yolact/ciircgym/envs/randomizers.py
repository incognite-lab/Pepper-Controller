...
repodir = pkg_resources.resource_filename("ciircgym", "")
...
class TextureRandomizer(Randomizer):
    ...
    def randomize(self):
        ...
        for object_id in textures_obj_ids:
            if object_id in self.env.scene_objects_uids.values() and self.env.scene_objects_uids[object_id] in self.exclude:
                pass
            else:
