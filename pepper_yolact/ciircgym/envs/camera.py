...
class Camera():
    ...
    def set_parameters(self, position=None, target_position=None,
        ...
        """
        if position is None: self.position = position
    ...
    def recompute_matrixes(self):
        ...
        if (self.is_absolute_position):
            self.view_matrix = self.p.computeViewMatrix(
                self.position, self.target_position, self.up_vector)
        else:
            self.view_matrix = self.p.computeViewMatrixFromYawPitchRoll(self.target_position, self.distance,
                                                                   self.yaw, self.pitch, self.roll,
                                                                   self.up_axis_index)
        self.proj_matrix = self.p.computeProjectionMatrixFOV(self.field_of_view, self.aspect_ratio,
    ...
    def get_opencv_camera_matrix_values(self):
        """
        Return values of OpenCV matrix
    ...
    def render(self):
        ...
        segmentation_mask_buffer = self.p.getCameraImage(viewMatrix=self.view_matrix,
